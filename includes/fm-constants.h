...
//
//	common functions
static inline bool isIndeterminate(DSPFLOAT x)
{
  return x != x;
}
...
{
  return x == std::numeric_limits<DSPFLOAT>::infinity();
}
//
...
static inline float get_db(DSPFLOAT x, int32_t y)
{
  return 20 * log10((x + 1) / (float)(y));
}

static inline DSPFLOAT PI_Constrain(DSPFLOAT val)
{
  if (0 <= val && val < 2 * M_PI)
  {
    return val;
