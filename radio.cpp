  ...
  , mSaveName(stationList)
{
  ...
  connect(&myLine, &QLineEdit::returnPressed, this, &RadioInterface::handle_myLine);
...
void RadioInterface::dumpControlState(QSettings *s)
{
  ...
  s->setValue("peakLevelDelaySteps", sbDispDelay->value());
...
void RadioInterface::setStart()
{
  ...
  connect(cbTestTone, &QCheckBox::clicked, this, [this](bool isChecked){ myFMprocessor->setTestTone(isChecked); });
  connect(sbDispDelay, qOverload<int>(&QSpinBox::valueChanged), this, &RadioInterface::set_display_delay);
...
{
  const int32_t freq = myRig->getVFOFrequency() + LOFrequency;
  const QString programName = myLine.text();
  fprintf(stderr, "adding %s %s\n", programName.toLatin1().data(), QString::number(freq / Khz(1)).toLatin1().data());
