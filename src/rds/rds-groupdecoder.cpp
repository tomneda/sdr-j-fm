...
rdsGroupDecoder::rdsGroupDecoder(RadioInterface * RI)
{
  ...
  connect(this, SIGNAL(setRadioText(const QString &)), MyRadioInterface, SLOT(setRadioText(const QString &)));
  //connect(this, SIGNAL(clearRadioText(void)), MyRadioInterface, SLOT(clearRadioText(void)));
  connect(this, SIGNAL(setAFDisplay(int, int)), MyRadioInterface, SLOT(setAFDisplay(int, int)));
  connect(this, SIGNAL(setMusicSpeechFlag(int)), MyRadioInterface, SLOT(setMusicSpeechFlag(int)));
  connect(this, SIGNAL(clearMusicSpeechFlag(void)), MyRadioInterface, SLOT(clearMusicSpeechFlag(void)));
...
void rdsGroupDecoder::reset(void)
{
  ...
  memset(textBuffer, 0, NUM_OF_CHARS_RADIOTEXT * sizeof(char));
  textBuffer[NUM_OF_CHARS_RADIOTEXT] = 0; // if textbuffer should be used directly for text output
...
void rdsGroupDecoder::addtoStationLabel(uint32_t index, uint32_t name)
{
  ...
  if ((int32_t)stationNameSegmentRegister + 1 == (1 << NUMBER_OF_NAME_SEGMENTS))
  {
    const QString stat2 = QString(stationLabel);
    emit setStationLabel(stat2);
    stationNameSegmentRegister = 0;
  }
}
...
void rdsGroupDecoder::additionalFrequencies(uint16_t blockContents)
{
  ...
  const int32_t f2 = (af1 != 250 && af2 > 0 && af2 < 205 ? af2 * 100 + 87500 : 0); // 250 == af2 is MF or LF frequency, ignore that
...
//    }
    ...
    textSegmentRegister = 0;
    //memset(textBuffer, (new_txtABflag == 0 ? '.' : ','), NUM_OF_CHARS_RADIOTEXT * sizeof(char));
    //emit setRadioText(textBuffer);
  }
  ...
  textSegmentRegister |= 1UL << currentSegment;

  // print only radio text if all data from begin are collected until now (but there may come more...)
  if (textSegmentRegister + 1 == (1UL << (currentSegment + 1)))
  {
    //filterAndSetRadioText(textBuffer, NUM_OF_CHARS_RADIOTEXT);
    filterAndSetRadioText(textBuffer, currentSegment * NUM_CHARS_PER_RTXT_SEGMENT);
  }
...
//  if (endF || (textSegmentRegister + 1 == (1 << NUM_OF_FRAGMENTS)))
//  {
//    prepareText(textBuffer, NUM_OF_CHARS_RADIOTEXT);
//    textSegmentRegister = 0;
//    memset(textBuffer, '-', NUM_OF_CHARS_RADIOTEXT * sizeof(char));
//  }
}
...
void rdsGroupDecoder::filterAndSetRadioText(char * v, int16_t length)
{
  int16_t i;
