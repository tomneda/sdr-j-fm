...
static inline int32_t nearestTwoPower(int16_t n)
{
  int32_t res = 1;
...
fft_scope::fft_scope(QwtPlot *plotfield, int16_t displaySize, int32_t scale,
                     int16_t rasterSize, int32_t spectrumSize,
                     int32_t SampleRate, int16_t freq)
  : Scope(plotfield, displaySize, rasterSize)
{
  double  temp;
  ...
  {
    Window[i] = 0.42
              - 0.50 * cos((2.0 * M_PI * i) / (spectrumFillpoint - 1))
  ...
  for (int16_t i = 0; i < displaySize; i++)
  {
    X_axis[i] = ((double)vfo - (double)MaxFrequency +
...
void fft_scope::addElementsandShow(DSPCOMPLEX *v, int16_t n)
{
  ...
  for (int16_t i = n; i < spectrumSize; i++)
  {
    spectrumBuffer[i] = 0;
  }
  spectrum_fft->do_FFT();
  ...
  for (int16_t i = 0; i < displaySize / 2; i++)
  {
    displayBuffer[displaySize / 2 + i] = 0;

    for (int16_t j = 0; j < Incr; j++)
    {
...
void fft_scope::addElement(DSPCOMPLEX v, int16_t t)
{
  ...
  {
    Scope::Display(X_axis, dummyBuffer, amplification,
...
{
  Scope::Display(X_axis, displayBuffer, amplification,
                 ((int32_t)vfo + needle) / (int32_t)scale);
}
...
void fft_scope::mapSpectrumtoDisplay(int16_t level, int32_t freq)
{
  ...
  for (int16_t i = 0; i < displaySize; i++)
  {
    ...
    for (int16_t j = 0; j < (int16_t)(5 * ratio); j++)
    {
      int32_t a_index = (int32_t)floor((5 * (bottom_index + i * ratio) + j) / 5);
...
static inline float get_ldb(float x)
{
...
void fft_scope::doAverage()
{
  if (averageCount > 0)
  {
    for (int16_t i = 0; i < displaySize; i++)
    {
      ...
      averageBuffer[i] = ((double)(averageCount - 1)) / averageCount * averageBuffer[i]
  ...
  else
  {
    for (int16_t i = 0; i < displaySize; i++)
    {
      ...
      averageBuffer[i] = ((double)(freq / 2 - 1)) / (freq / 2) * averageBuffer[i]
