 ...
 */
Scope::Scope(QwtPlot *scope, uint16_t displaysize, uint16_t rastersize)
{
  Plotter       = scope;
                    ...
                    int32_t marker)
{
  ...
  {
    Waterfall->ViewWaterfall(x_axis, buffer, amp, marker);
  }
  else
  {
    Spectrum->ViewSpectrum(x_axis, buffer, amp, marker);
  }
}
...
SpectrumViewer::SpectrumViewer(QwtPlot *plot, uint16_t displaysize)
{
  ...
  connect(lm_picker, SIGNAL(selected(const QPointF&)), this,
  ...
  connect(rm_picker, SIGNAL(selected(const QPointF&)), this,
...
{
  disconnect(lm_picker, SIGNAL(selected(const QPointF&)), this,
             SLOT(leftMouseClick(const QPointF&)));
  disconnect(rm_picker, SIGNAL(selected(const QPointF&)), this,
             SLOT(rightMouseClick(const QPointF&)));
...
void SpectrumViewer::ViewSpectrum(double *X_axis, double *Y1_value, double amp,
                                  int32_t markerValue)
{
  uint16_t i;
 ...
 */
WaterfallViewer::WaterfallViewer(QwtPlot *plot, uint16_t displaysize,
                                 uint16_t rastersize)
  : QwtPlotSpectrogram()
{
  int i, j;
  ...
  WaterfallData =
    new SpectrogramData(plotData, 10000, 1000, Rastersize, Displaysize, 50.0);
                                    ...
                                    double amp, int32_t marker)
{
   ...
   */
  memcpy(&plotData[(Rastersize - 1) * Displaysize], &Y1_value[0],
  ...
  WaterfallData =
    new SpectrogramData(plotData, orig, width, Rastersize, Displaysize, amp);
  //	this		-> detach	();
  this->setData(WaterfallData);
  this->setDisplayMode(QwtPlotSpectrogram::ImageMode, true);
